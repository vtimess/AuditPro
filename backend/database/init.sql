...
CREATE TABLE IF NOT EXISTS `ops_personnel` (
  `id` BIGINT NOT NULL AUTO_INCREMENT,
  `staff_no` VARCHAR(20) NULL COMMENT '自动生成序号sy001',
  `unit_name` VARCHAR(64) NOT NULL DEFAULT '泗阳队',
  `department_name` VARCHAR(64) NOT NULL DEFAULT '泗阳劳务',
  ...
  `nationality` VARCHAR(32) NOT NULL DEFAULT '汉族',
  `education` VARCHAR(32) NOT NULL DEFAULT '小学',
  `chronic_disease` VARCHAR(32) NOT NULL DEFAULT '否',
  ...
  `political_status` VARCHAR(32) NOT NULL DEFAULT '群众',
  `joined_party_at` DATE NULL,
  `served_in_military` TINYINT(1) NOT NULL DEFAULT 0,
  `military_service_time` VARCHAR(64) NULL,
  `port_entry_date` DATE NULL,
  ...
  `accident_insurance_limit` VARCHAR(20) NOT NULL DEFAULT '150+',
  `created_by` BIGINT NOT NULL,
  `record_status` VARCHAR(20) NOT NULL DEFAULT 'active',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_personnel_staff_no` (`staff_no`),
  UNIQUE KEY `uk_personnel_id_card_hash` (`id_card_hash`),
  KEY `idx_personnel_name_status` (`person_name`, `employment_status`),
  KEY `idx_personnel_mobile` (`mobile`),
  CONSTRAINT `fk_personnel_created_by` FOREIGN KEY (`created_by`) REFERENCES `sys_user` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='人员花名册';
